...
class Vector:
    ...
    def __sub__(self, vec):  # разность векторов
        return self.x[0] - vec.x[0], self.x[1] - vec.x[1]

    def __add__(self, vec):  # сумма векторов
        return self.x[0] + vec.x[0], self.x[1] + vec.x[1]
...
if __name__ == "__main__":
    ...
    while working:
        ...
        Line.draw_points(Joint(points, steps).get_joint(), "line", 4, color)
        if not pause:
